...
SelectedSession = sessions(1); % please run the decoding on all 3 sessions - but start with session 2
...
for f = 1:length(folders)
    ...
    save(folders(f)+"binned_activity_decode_early_running.mat","a_use","context_use","corr_coef","rate","-mat");
end



%% additional function to label resting periods - resting before first running period in trial is excluded to avoid reward consumptions
function a = GetRestState(a)
NTrial = length(a.TrialFrames(:,1));
a.Rest = zeros(length(a.States.Run),1);
for i = 1:NTrial
    ...
    a.Rest(ForagingStart:ForagingStop) = TrialRest;
end
end