...
function [All,Individual]= WrapVectorRemap2(workdir, discrimination, LightBin)
...
for f = 1:length(workdir)  
    disp(workdir(f))
    clearvars -except f All workdir discrimination Individual LightBin
    ...
    % light cue tuning
    [IsLightTuned, WhichLight, IsMatch] = GetLightTuning(TunedBin, LightBin,Rew.Bin);
    All.LightTuning{1} = cat(1,All.LightTuning{1}, IsLightTuned);
    All.MatchLight{1} = cat(1,All.MatchLight{1}, IsMatch);
    All.WhichLight{1} = cat(1,All.WhichLight{1}, WhichLight);
    ...
    [IsLightTuned, WhichLight, IsMatch] = GetLightTuning(TunedBin, LightBin,Rew.Bin);
    All.LightTuning{2} = cat(1,All.LightTuning{2}, IsLightTuned);
    All.MatchLight{2} = cat(1,All.MatchLight{2}, IsMatch);
    All.WhichLight{2} = cat(1,All.WhichLight{2}, WhichLight);
