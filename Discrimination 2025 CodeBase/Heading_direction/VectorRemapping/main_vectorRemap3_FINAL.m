...
workdir_d = ["H:\Included miniscope Mice\M119\TrainingD11\"  "H:\Included miniscope Mice\M120\TrainingD11\"  "H:\Included miniscope Mice\M292\TrainingD6\"  "H:\Included miniscope Mice\M319\TrainingD7\" "F:\Grouping First\M231\TrainingD9\" "F:\Grouping First\M314\Training_Separation_D5\" "F:\Grouping First\M316\Training_Separation_D6\"  "F:\Grouping First\M318\Training_Separation_D4\" "H:\Included miniscope Mice\M210\TrainingD17\"];
...
workdir_g = ["H:\Included miniscope Mice\M119\GroupingD6\" "H:\Included miniscope Mice\M120\GroupingD6\" "H:\Included miniscope Mice\M292\GroupingD3\" "H:\Included miniscope Mice\M319\GroupingD4\" "F:\Grouping First\M231\GroupingD5\" "F:\Grouping First\M314\GroupingD3\" "F:\Grouping First\M316\GroupingD3\" "F:\Grouping First\M318\GroupingD3\" ];
...
LightBins_d = [5 6; 3 4];
[All_d, Individual_d]= WrapVectorRemap3_FINAL(workdir_d, discrimination, LightBins_d);
LightBins_g = [1 2; 7 8];
[All_g, Individual_g]= WrapVectorRemap3_FINAL(workdir_g, discrimination, LightBins_g);
...
%% note some cells are double-counted because the're tuned to one category in one ctxt and to another in the other ctxt (rare)
figure
tiledlayout(2,3)
...
%% field analysis of reward tuned HDCs
figure
tiledlayout(1,2)
nexttile
bar(1, sum(All_d.HasRewardField, "omitnan")/sum(~isnan(All_d.HasRewardField))*100)
hold on
bar(2, sum(All_g.HasRewardField, "omitnan")/sum(~isnan(All_g.HasRewardField))*100)
box off
axis square
ylim([0 100])
...
%% calculate corrcoef beween A and B
remap_d = GetRemap(All_d.Curves);
remap_g = GetRemap(All_g.Curves);
[yd,xd] = ecdf(remap_d );
[yg,xg] = ecdf(remap_g );
...
%% plot tuning bins
TunedBin_d = [All_d.TuningBin{1}; All_d.TuningBin{2}];
onlyA = find(~isnan(TunedBin_d(:,1) ) & isnan(TunedBin_d(:,2) ));
onlyB = find(isnan(TunedBin_d(:,1) ) & ~isnan(TunedBin_d(:,2) ));
tunedA = histcounts(TunedBin_d(onlyA, 1),1:9);
tunedB = histcounts(TunedBin_d(onlyB, 2), 1:9);
...
tiledlayout(2,2)
nexttile;
bar(1:8, tunedA./sum(tunedA).*100, "FaceColor","k")
hold on
bar([5 6], tunedA(5:6)./sum(tunedA).*100, "FaceColor","y")
xticklabels(wallsDir)
ylabel("% of HDCs")
box off
axis square
title("Tuned in ctxt A")

nexttile;
bar(1:8, tunedB./sum(tunedB).*100, "FaceColor","k")
hold on
bar([3 4], tunedB(3:4)./sum(tunedB).*100, "FaceColor","y")
xticklabels(wallsDir)
ylabel("% of HDCs")
box off
axis square
title("Tuned in ctxt B")

TunedBin_g = [All_g.TuningBin{1}; All_g.TuningBin{2}];
onlyA = find(~isnan(TunedBin_g(:,1) ) & isnan(TunedBin_g(:,2) ));
onlyB = find(isnan(TunedBin_g(:,1) ) & ~isnan(TunedBin_g(:,2) ));
tunedA = histcounts(TunedBin_g(onlyA, 1),1:9);
tunedB = histcounts(TunedBin_g(onlyB, 2), 1:9);

nexttile;
bar(1:8, tunedA./sum(tunedA).*100, "FaceColor","k")
hold on
bar([1 2], tunedA(1:2)./sum(tunedA).*100, "FaceColor","y")
xticklabels(wallsDir)
ylabel("% of HDCs")
box off
axis square
title("Tuned in ctxt C")

nexttile;
bar(1:8, tunedB./sum(tunedB).*100, "FaceColor","k")
hold on
bar([7 8], tunedB(7:8)./sum(tunedB).*100, "FaceColor","y")
xticklabels(wallsDir)
ylabel("% of HDCs")
box off
axis square
title("Tuned in ctxt D")
...
function remap = GetRemap(Curves)

[N_cell,~,~] = size(Curves);
remap = nan(N_cell,1);
for i = 1:N_cell
    remap(i) = nancorr(Curves(i,:,1), Curves(i,:,2));
end
end