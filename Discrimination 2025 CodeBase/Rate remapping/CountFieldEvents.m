%% Function to count Ca events in individual fields
function [EventCounter, FieldEventTime, FieldEventPercentage] = CountFieldEvents(FieldMap_full, onset, Field_tbl, p)
...
field_counter = 1;
    ...
    % have to be matched
    non_drift = find(onset.PC_tbl.IsDrifting ~= 1); 
    pc_id = find(onset.PC_tbl.IsPC(:,1) == 1); % place cell id
    [~, matched_id, ~] =  intersect(non_drift, pc_id);
for i =1:length(FieldMap_full)
    
    % next concat eventXY for the current cell
    EventBins = [onset.EventXY{matched_id(i),1}; onset.EventXY{matched_id(i),2}];
    EventTs = [onset.EventTime{matched_id(i),1}; onset.EventTime{matched_id(i),2}];
