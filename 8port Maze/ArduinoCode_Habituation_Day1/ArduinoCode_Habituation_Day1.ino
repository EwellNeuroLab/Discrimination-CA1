    ...
    for(int port = 0; port <=7; port++){
     ...
     //if IR is broken and has been broken before and this port has not been visited right before this detection 
      ...
      if(SensorState > IRThreshold[port] && PortFlags[port] == 0){
        ...
        Serial.println("Correct_"+ Position[port] + "_"+ VisitCounter); //send message to Bonsai which one was visited 
         int sumVisit = 0;
         for(int i = 0; i<= 7;i++){
          sumVisit += PortFlags[i];
         }     
        if(PortCounter == 8 ){ // if all the 8 ports were visited
          Serial.println("End_"+ Position[port]+"_"+ VisitCounter);
