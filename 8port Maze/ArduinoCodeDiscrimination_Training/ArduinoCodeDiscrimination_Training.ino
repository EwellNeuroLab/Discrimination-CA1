// Gergely Tarcsay, 2025. Arduino code for the discrimination task training. Parameters are initialized for 8 mouse ports and 8 light strip.
...
// PreTrainingI, beside setting TrainingBlock to 800 (line 9), set MaxStrike to a number that is higher than number of ports (e.g. 10). That way mouse cannot make incorrect trials even when it pokes every port (pokes are not double-counted).
